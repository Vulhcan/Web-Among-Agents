...
# Example roster used when no --crewmate_llm / --impostor_llm is passed.
# Edit freely: any model id your OpenRouter account can reach will work.
...
ARGS = {
    ...
    "agent_config": {
        "Impostor": "LLM",
        "Crewmate": "LLM",
        "IMPOSTOR_LLM_CHOICES": DEFAULT_MODELS,
        "CREWMATE_LLM_CHOICES": DEFAULT_MODELS,
    },
    "UI": False,
}
    ...
    async def run_limited_game(game_index):
        async with semaphore:
            if ARGS.get("tournament_style") == "1on1":
                ...
                game_config["CREWMATE_LLM_CHOICES"] = [random.choice(DEFAULT_MODELS)]
                game_config["IMPOSTOR_LLM_CHOICES"] = [random.choice(DEFAULT_MODELS)]
            else:
